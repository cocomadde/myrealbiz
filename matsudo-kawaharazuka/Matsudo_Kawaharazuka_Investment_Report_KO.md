...
| **연간 원리금상환액 (ADS)** | **404.9만 엔** (¥4,049,064) | LTV 80.0% (7,520만 엔), 금리 2.5%, 25년 원리금균등 |
| **세전 캐시플로 (만실 시)** | **+117.2만 엔/년** (+¥1,171,707) | 월 약 +9.76만 엔의 안정적 잉여현금 |
| **DSCR (부채상환배율)** | **1.29배** | ✅ 안전 기준선(1.20배) 안정적 상회 |
| **손익분기 가동률** | **82.40%** | ✅ 허용 공실 호수 **1.41실** (1실 공실 시에도 흑자 유지) |
...
| **초기 필요 자기자본** | **2,669.2만 엔** (¥26,692,000) | 자기자본 두금 1,880만 엔 + 취득제비용 789.2만 엔 |
| **CCR (자기자본수익률)** | **4.39%** | 세전 잉여현금흐름 / 초기 총 투하자기자본 |
...
| :--- | :---: | :---: | :---: |
| **1. 帳簿上税引前当期純利益 (장부상 세전순이익)** | ¥-1,829,413 | ¥3,722,450 | **¥1,893,037** |
| **2. 税引前手残りCF (세전 잔여 현금흐름)** | ¥864,106 | ¥1,741,901 | **¥2,606,007** |
| 　・**税引前CF利回り (세전 CF 수익률)** | 0.92% | 0.65% | **0.72%** |
| 　・**推定法人税額 (추정 법인세액)** | ¥70,000 (균등할) | ¥930,613 | **¥473,259** (절세효과) |
| 　・**税引後手残りCF (세후 잔여 현금흐름)** | ¥794,106 | ¥811,289 | **¥2,132,748** |
| 　・**税引後CF利回り (세후 CF 수익률)** | 0.84% | 0.30% | **0.59%** |
| **3. 債務償還年数 (채무상환년수, 년)** | 17.9년 | 24.1년 | **21.5년** (건전성 대폭 개선) |
| **4. 簡易 Cash Flow (=税引後CF)** | ¥794,106 | ¥811,289 | **¥2,132,748** |
| **5. ICR (이자보상배율, Interest Coverage Ratio)** | -0.6배 | 0.9배 | **0.6배** |
| **6. 負債返済割合 (DSCR / 부채상환배율)** | 1.5배 | 1.5배 | **1.5배** |
| **7. LTV (固定資産税評価額に基づく / 고정자산세 기준)**| 223.02% | 261.08% | **250.89%** |
| **8. 投資額ROI (초기 투하자기자본 대비 ROI)** | 3.40% | 3.83% | **3.67%** |

> [!TIP]
> **법인 절세 및 현금흐름 시너지**:
> 본 물건은 30년 초과 철골조 건물로 법정내용연수가 만료되어 건물분(약 1,880만 엔)을 5년 단기 감가상각(연간 약 376만 엔 계상)할 수 있습니다. 이를 통해 법인 단계에서 회계상 장부 적자를 유도하여 **기존 모아비즈 법인의 법인세 부담을 연간 약 45.7만 엔 절감**하면서도, 실제 **세후 잔여 현금흐름은 연 213.3만 엔(월 약 17.8만 엔)으로 순증**하는 최적의 자산 착지가 가능합니다.
...
| **소재지** | 치바현 마츠도시 카와하라즈카 29-1, 29-4, 29-7, 29-8 (千葉県松戸市河原塚29-1外) |
| **교통 접근성** | • **JR 무사시노선 「신야하시라(新八柱)」역 도보 10~11분**<br>• **신경성선 「야하시라(八柱)」역 도보 10~11분**<br>• JR 무사시노선 「히가시마츠도(東松戸)」역 도보 25분 |
...
| 지표 항목 | 시나리오 A (표준 LTV 80%) | 시나리오 B (보수 안전 LTV 70%) | 시나리오 C (토지담보 장기 25년) [채택] |
| :--- | :---: | :---: | :---: |
| **차입금액 (대출금)** | ¥75,200,000 (80%) | ¥65,800,000 (70%) | **¥75,200,000 (80%)** |
| **차입 금리 / 기간** | 2.5% / 20년 | 2.5% / 20년 | **2.5% / 25년** |
| **월 원리금 상환액** | ¥398,735 | ¥348,893 | **¥337,422** |
| **연간 부채상환액 (ADS)** | ¥4,784,820 | ¥4,186,716 | **¥4,049,064** |
| **만실 순영업소득 (NOI)** | ¥5,220,771 | ¥5,220,771 | **¥5,220,771** |
| **세전 연간 캐시플로 (CF)** | **+¥435,951** | **+¥1,034,055** | **+¥1,171,707** (월 +9.76만) |
| **DSCR (부채상환배율)** | 1.09배 | **1.25배** | **1.29배** (안전 기준 충족) |
| **손익분기 가동률** | 93.45% | 84.46% | **82.40%** |
| **허용 공실 호수** | 0.52실 (위험) | 1.24실 (양호) | **1.41실 (1실 공실 안전)** |
| **필요 자기자본 (두금+제비용)** | ¥26,692,000 | ¥36,092,000 | **¥26,692,000** |
| **자기자본수익률 (CCR)** | 1.63% | 2.87% | **4.39%** |
...
### 9.1 입지 및 교통 인프라
- **더블 역세권의 뛰어난 직주근접성**:
  - JR 무사시노선 「신야하시라」역: 도쿄 동부 및 사이타마/치바 순환선. 도쿄역까지 약 35분 직통(게이요선 직결 열차 이용 시).
  - 신경성선 「야하시라」역: 마츠도역에서 JR 조반선(도쿄메트로 치요다선 직결) 환승 용이.
- **풍부한 생활 편의시설**:
...
| 평가 부문 | 배점 | 득점 | 평가 요약 |
| :--- | :---: | :---: | :--- |
| **1. 수익성 및 현금흐름 (Cash Flow)** | 5.0 | **3.5** | 만실 표면수익률 7.08%, 실질 NOI 5.55%, 연간 세전 CF 117만 엔의 견실한 현금흐름. |
...
| **합계 (Total Score)** | **30.0** | **24.0** | **투자 등급: A (적극 매수 추천 / BUY)** |
...
> 건물의 노후화와 우물물 설비라는 실사 포인트가 존재하지만, 이를 감안하고도 **189.67평의 남북 양면 도로 대형 토지**가 주는 자산 가치와 **연 117만 엔 이상의 안정적 현금흐름**, 그리고 **법인 단기 감가상각(5년)을 통한 절세 시너지**가 매우 뛰어납니다.
...
3. **토지 담보 25년 융자 확보**: 치바은행, 치바흥업은행, 또는 SBJ은행/오릭스은행을 통해 토지 담보력 기반 25년 장기 융자(LTV 80%) 승인 추진.
