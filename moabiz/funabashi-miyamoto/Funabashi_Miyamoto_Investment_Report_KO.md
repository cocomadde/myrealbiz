...
| **9. 法人貢献総合判定 (모아비즈 법인 편입 적격성 종합 판정)** | **조건부 기여** | 기준선 (ICR 0.9배 취약) | **조건부 적합 (순이익·ICR·CF 개선 / ROI·세무 조건 충족 필수)** |

> [!NOTE]
> **모아비즈 법인 편입 적격성 5대 핵심 검증 (ICR / DSCR / ROI / 장부순이익 / 캐시플로우)**:
> 1. **ICR (이자보상배율)**: 기존 0.9배(취약) -> **1.0배로 소폭 개선** (1.0배 턱걸이 진입으로 최소 금융비용 방어선 확보).
> 2. **DSCR (부채상환배율)**: **1.5배 안정적 유지** (단품 1.4배, 합산 1.5배로 은행 권고 기준 1.20배 이상 유지).
> 3. **ROI (자기자본수익률)**: 기존 3.83% -> **3.60%로 -0.23%p 소폭 희석** (신축 목조 프리미엄으로 초기 자기자금 대비 CF 얇음).
> 4. **장부상 당기순이익**: 연간 372.2만 엔 -> **604.9만 엔으로 +232.6만 엔(1.62배) 증가** (결산서상 흑자 기여).
> 5. **실질 캐시플로우 (CF)**: 세후 순유입 81.1만 엔 -> **98.4만 엔으로 +17.3만 엔 순증** (월 약 1.4만 엔 수준으로 마진 얇음).
> 
> **종합 판정 결론**: **조건부 적합 (Conditional Fit)** — 장부상 세전순이익(+232.6만 엔)과 ICR(1.0배 진입), 연간 CF(+17.3만 엔)가 순증하여 법인 결산서 개선에 기여하지만, ROI 소폭 희석 및 얇은 CF 마진으로 인해 ① 25년 이상 장기 융자, ② 금리 2.3% 이하, ③ 감가상각을 통한 법인세 절세 설계, ④ 매매가 가격 절충(1억 300만 엔 선) 등 전제 조건이 충족될 때에 한하여 법인 편입이 권고됨.
