...
> ※ 본 분석은 당사 법인의 재무 건전성 및 여신 한도 극대화를 위해, 기본 베이스라인인 **「모아비즈 4기 현재(Col G)」** 결산 실적에 본 물건을 신규 편입했을 때의 연결 재무제표 착지 효과를 3열 정밀 모델링으로 검증한 결과입니다.

| 항목 (指標項目) | 검증대상 부동산 단품 (Standalone) | 모아비즈 4기 현재 (Baseline) | 본 물건 편입 후 결산서 착지 예상 (Merged) | 착지 판정 및 기여도 |
| :--- | :---: | :---: | :---: | :--- |
| **1. 장부상 세전당기순이익** | **+90.3만 엔** | ¥3,722,450 | **¥4,625,174** | **+90.3만 엔 (+24.2%) 순증 달성** |
| **2. 세전 잔여 현금흐름 (Pre-tax CF)** | **+216.1만 엔** | ¥1,741,901 | **¥3,903,319** | **+216.1만 엔 (+124.1%) 폭증** |
| 　・세전 CF 수익률 | 1.37% | 0.65% | **0.92%** | +27bps 개선 |
| 　・추정 법인세액 (25% 가정) | ¥225,681 | ¥930,613 | **¥1,156,294** | 흑자 결산에 따른 정상 법인세 |
| 　・**세후 잔여 현금흐름 (Post-tax CF)**| **+193.6만 엔** | ¥811,289 | **¥2,747,025** | **+193.6만 엔 (+238.6%, 3.38배 폭증)** |
| 　・세후 CF 수익률 | 1.23% | 0.30% | **0.65%** | +35bps 개선 |
| **3. 채무상환년수 (Debt Years)** | 21.9년 | 24.1년 | **26.2년** | 30년 융자 추가 대비 안정적 통제 |
| **4. 간이 Cash Flow (=세후CF)** | **+193.6만 엔** | ¥811,289 | **¥2,747,025** | 순수 사내 유보 잉여현금 대폭 확충 |
| **5. 이자보상배율 (ICR)** | **2.67배** | 0.90배 | **2.03배** | **기존 0.9배 취약선 탈피, 2.0배 우량선 안착** |
| **6. 부채상환배율 (DSCR)** | **1.33배** | 1.50배 | **1.20배** | 전사적 1.20배 적정 가이드라인 완벽 수호 |
| **7. 고정자산세 평가 기반 LTV** | **138.4%** | 261.08% | **176.9%** | **-84.2%p 극적 축소! 재무건전성 획기적 개선** |
| **8. 투자액 대비 ROI** | **7.21%** | 3.83% | **4.78%** | 전사 자본효율성 +95bps 향상 |
...
flowchart LR
    ...
    A --> E["재무구조 개선: 9.0 / 10<br/>(ICR 2.03배, LTV -84%p)"]
```
...
| **6. 법인 재무제표 기여도** | **최고 적합** | 취약했던 법인 ICR을 0.90배에서 **2.03배**로 정상화하고, 세후 잉여CF를 **3.38배(연 +194만 엔)** 폭증시킴. |
...
- **더블 노선 접근성 및 구글맵 도보 실측**:
  - 케이세이 마츠도선(구 신경성선) **「마에바라」역**: 공시 도보 13분, **구글맵 실측 1.05km (도보 13~14분)**. 완만한 평탄지 주택가 도로를 통과하며 신호 1~2회로 공시 시간과 실측 시간이 정확히 일치함.
  - 마에바라역에서 1정거장(2분) 거리인 **「신츠다누마(新津田沼)」역을 통해 JR 총부쾌속선 츠다누마역(도쿄역 28분, 시나가와·신주쿠 직통)**과 즉각 연계.
  - **「야쿠엔다이」역**: 도보 18분 (실측 1.40km, 도보 18~19분).
  - **JR 「츠다누마」역 직접 접근성**: 실측 약 2.1km(도보 27~28분, 자전거 약 8~9분). 도보 2분 거리 버스정류장(「타키노이 1초메」)에서 츠다누마역 북구행 버스가 출퇴근 시간대 5~10분 간격으로 운행하여 실질 대중교통 이용 편의성이 우수함.
- **평일 출근시간대(07:00~09:00 출발) 도쿄 4대 도심 핵심역 Door-to-Door 통근 동선 분석**:
  ...
  | :--- | :---: | :--- | :---: | :---: | :---: | :--- |
  | **도쿄(東京)역** (비즈니스/금융) | **약 46~48분** | 집 도보 2분 + 버스 10분(또는 자전거 8분) → JR 츠다누마역 + JR 총부쾌속선 직통 31분 | 도보/버스 12분 | 31분 | **0회 (전철 직통)** | 츠다누마역 시발(始發) 총부쾌속 열차가 다수 배차되어 착석 통근 가능, 도쿄역 지하 플랫폼 직결 |
  | **오테마치(大手町)역** (오피스/지하철결절) | **약 45~48분** | 집 도보 2분 + 버스 10분 → 츠다누마역 + JR 총부선 니시후나바시(6분) → 도쿄메트로 토자이선 통근쾌속(23분) | 도보/버스 12분 | 29분 | 1회 | 토자이선 통근쾌속으로 오테마치역 핵심 오피스 빌딩군 다이렉트 도달 |
  | **신주쿠(新宿)역** (최대 터미널) | **약 55~58분** | 집 도보 2분 + 버스 10분 → 츠다누마역 + JR 츄오·총부선 각역정차 직통(48분) 또는 킨시초 환승 | 도보/버스 12분 | 42~48분 | 0~1회 | 츠다누마 시발 각역정차 열차 착석 통근 또는 쾌속 환승으로 이동 편리 |
  | **시부야(渋谷)역** (IT/트렌드) | **약 60~65분** | 집 도보 2분 + 버스 10분 → 츠다누마역 + JR 총부쾌속 시나가와역(40분) → JR 야마노테선(12분) | 도보/버스 12분 | 52분 | 1회 | 시나가와 환승 야마노테선 또는 도쿄역 환승 마루노우치/긴자선 연계 |
- **생활 인프라**:
